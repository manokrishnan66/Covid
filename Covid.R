...
C <- read.csv("result.csv",stringsAsFactors = FALSE)

tidy_data_c <- C %>%
  gather(State, count, `An`:`wb`)

Data_dft <- tidy_data_c %>% spread(status, count)
summary(Data_dft)
# 
...
###---- Splitting of datasets-Covid

set.seed(1, sample.kind="Rounding")
# if using R 3.5 or earlier, use `set.seed(1)` instead
Valid_index <- createDataPartition(y = Data_dft$Confirmed, times = 1, p = 0.2, list = FALSE)
Data_dft_main <- Data_dft[-Valid_index,]
Data_dft_validation <- Data_dft[Valid_index,]

# Validation set will be 20% of Portdata_main data
set.seed(2, sample.kind="Rounding")
# if using R 3.5 or earlier, use `set.seed(1)` instead
test_index <- createDataPartition(y = Data_dft_main$Confirmed, times = 1, p = 0.2, list = FALSE)
Data_dft_main_train <- Data_dft_main[-test_index,]
Data_dft_main_test <- Data_dft_main[test_index,]
...
##GLM:
summary(Data_dft)
model_rf = randomForest(Confirmed ~ .,
                        data = Data_dft_main_train)
